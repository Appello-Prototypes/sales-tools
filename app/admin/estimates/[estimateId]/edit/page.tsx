...
import {
  ...
  Building2, User, Search, Loader2, X,
...
export default function EditEstimatePage() {
  ...
  const fetchEstimate = async () => {
    try {
      ...
      setCompanyName(estimate.preparedFor.companyName);
      setContactName(estimate.preparedFor.contactName);
      setContactEmail(estimate.preparedFor.email || '');
      setContactPhone(estimate.preparedFor.phone || '');
      setCompanyAddress(estimate.preparedFor.address || '');
      setNumberOfUsers(estimate.numberOfUsers);
      ...
      // Set HubSpot IDs
      setSelectedDealId(estimate.hubspotDealId || null);
      setSelectedCompanyId(estimate.hubspotCompanyId || null);
      setSelectedContactId(estimate.hubspotContactId || null);
      
      // Set line items
      if (estimate.lineItems && estimate.lineItems.length > 0) {
        setLineItems(estimate.lineItems);
      } else {
        initializeLineItems(estimate.numberOfUsers);
      }
      
      // Set discounts from stored data
      if (estimate.discounts && estimate.discounts.length > 0) {
        estimate.discounts.forEach(d => {
          if (d.appliedTo === 'onboarding') {
            setOnboardingDiscount({ enabled: d.enabled, percentage: d.percentage, name: d.name });
          } else if (d.appliedTo === 'users') {
            setUserDiscount({ enabled: d.enabled, percentage: d.percentage, name: d.name });
          } else if (d.appliedTo === 'server') {
            setServerDiscount({ enabled: d.enabled, percentage: d.percentage, name: d.name });
          } else if (d.appliedTo === 'modules') {
            if (d.id === 'early_adopter') {
              setModuleDiscount({ enabled: d.enabled, percentage: d.percentage, name: d.name });
            } else if (d.id === 'nia') {
              setNiaDiscount({ enabled: d.enabled, percentage: d.percentage, name: d.name });
            }
          }
        });
      }
  ...
  const initializeLineItems = (users?: number) => {
    const numUsers = users || numberOfUsers;
    const tier = getTierForUsers(numUsers);
    ...
    items.push({
      ...
      discountedPrice: numUsers * USER_COST_BASE,
    });
    ...
    DEFAULT_MODULES.forEach(module => {
      items.push({
        ...
        notes: module.free ? 'free with all subscriptions' : (module as { badge?: string }).badge,
      });
    });
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      // Then fetch the PDF
      const pdfResponse = await fetch(`/api/admin/estimates/${estimateId}/pdf`);
      if (pdfResponse.ok) {
        ...
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Link href={`/admin/estimates/${estimateId}`} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
                <ArrowLeft className="h-5 w-5" />
              </Link>
            <div>
                <h1 className="text-lg font-semibold text-gray-900">Edit Estimate</h1>
