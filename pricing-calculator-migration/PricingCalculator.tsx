...
First Year Total: ${formatCurrency(calculations.onboardingCost + (paymentFrequency === "annual" ? calculations.paymentAmount : paymentFrequency === "quarterly" ? calculations.paymentAmount * 4 : calculations.monthlySubscriptionSubtotal * 12))}`;
  ...
  return (
    <div id="pricing-calculator" className="flex flex-col lg:flex-row gap-6 max-w-7xl mx-auto items-start relative">
      {/* Main Content */}
      <div className="flex-1 bg-white rounded-xl border-2 border-gray-200 p-6 md:p-8 shadow-xl">
      {/* Header */}
      <div className="mb-8">
        ...
        <div className="sticky top-20 pb-6">
          <div className="bg-white rounded-xl border-2 border-gray-200 shadow-xl p-6">
            ...
            <div className="space-y-4 mb-6">
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Configuration</div>
                <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Users</span>
                    <span className="font-semibold text-gray-900">{numUsers}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Tier</span>
                    <span className="font-semibold text-gray-900">{currentTier.tier}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Modules</span>
                    <span className="font-semibold text-gray-900">{calculations.selectedModulesCount}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Onboarding */}
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Onboarding</div>
                <div className="bg-green-50 rounded-lg p-3 border border-green-200">
                  <div className="flex justify-between items-center">
                    <div>
                      <div className="text-xs text-gray-500 line-through">{formatCurrency(currentTier.onboarding)}</div>
                      <div className="text-lg font-bold text-green-600">{formatCurrency(calculations.onboardingCost)}</div>
                    </div>
              ...
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Monthly Subscription</div>
                <div className="bg-blue-50 rounded-lg p-3 border border-blue-200 space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-600">Users</span>
                    <span className="font-medium text-gray-900">{formatCurrency(calculations.totalUserCost)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Server & Storage</span>
                    <span className="font-medium text-gray-900">{formatCurrency(calculations.serverStorageCost)}</span>
                  </div>
                  {calculations.selectedModulesCount > 0 && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Modules</span>
                      <span className="font-medium text-gray-900">{formatCurrency(calculations.totalModuleCost)}</span>
                    </div>
                  )}
                  <div className="pt-2 border-t border-blue-200 flex justify-between font-bold">
                    <span className="text-gray-700">Subtotal</span>
                    <span className="text-primary">{formatCurrency(calculations.monthlySubscriptionSubtotal)}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                    <span className="text-gray-500">Tax (HST 13%)</span>
                    <span className="text-gray-500">{formatCurrency(calculations.taxAmount)}</span>
                  </div>
                  <div className="pt-2 border-t-2 border-blue-300 flex justify-between font-bold text-lg">
                    <span className="text-gray-900">Total</span>
                    <span className="text-primary">{formatCurrency(calculations.monthlySubscriptionWithTax)}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Payment Frequency */}
              <div>
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Payment Plan</div>
                <div className={`rounded-lg p-3 border-2 ${
                  paymentFrequency === "monthly" ? "bg-gray-50 border-gray-300" :
                  paymentFrequency === "quarterly" ? "bg-blue-50 border-blue-300" :
                  "bg-green-50 border-green-300"
                }`}>
                  <div className="flex justify-between items-center mb-2">
                    <span className="font-semibold text-gray-900 capitalize">{paymentFrequency}</span>
                    {paymentFrequency !== "monthly" && (
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        paymentFrequency === "quarterly" ? "bg-blue-100 text-blue-700" : "bg-green-100 text-green-700"
                      }`}>
                        {paymentFrequency === "quarterly" ? "8.3% OFF" : "16.7% OFF"}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="text-xs text-gray-500">per month equivalent</div>
                  {paymentFrequency !== "monthly" && (
                    <div className="mt-2 pt-2 border-t border-gray-200">
                      <div className="text-xs text-gray-600">Annual: {formatCurrency(calculations.annualEquivalent)}</div>
                      {calculations.savings > 0 && (
                        <div className="text-xs font-medium text-green-600 mt-1">
                          Save {formatCurrency(calculations.savings)}/year
                        </div>
                      )}
                    </div>
                  )}
            ...
            {/* First Year Total */}
            <div className="bg-gradient-to-br from-primary/10 to-primary/5 rounded-lg p-4 border-2 border-primary mb-4">
              <div className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">First Year Total</div>
            ...
            {selectedModulesData.length > 0 && (
              <div className="mb-4">
                <div className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 flex items-center justify-between">
                  ...
                  <span className="text-primary font-bold">{selectedModulesData.length}</span>
                </div>
                <div className="space-y-1.5">
                  {selectedModulesData.map((module) => {
                    const modulePrice = module.basePrice > 0 
                      ? module.basePrice * (1 - EARLY_ADOPTER_DISCOUNT) * (hasNIADiscount ? (1 - NIA_DISCOUNT) : 1)
                      : 0;
                    return (
                      <div key={module.id} className="flex items-center justify-between text-xs bg-gray-50 rounded-lg px-3 py-2 border border-gray-200 hover:bg-gray-100 transition-colors">
                        <span className="text-gray-700 truncate flex-1 font-medium">{module.name}</span>
                        {modulePrice > 0 && (
                          <span className="text-gray-600 font-semibold ml-2 whitespace-nowrap">
            ...
            {/* Discounts Applied */}
            {(hasNIADiscount || calculations.earlyAdopterDiscount > 0) && (
              <div className="mb-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                <div className="text-xs font-semibold text-yellow-800 uppercase tracking-wide mb-2">Discounts Applied</div>
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between text-yellow-700">
                    <span>Early Adopter (20%)</span>
                    <span className="font-semibold">-{formatCurrency(calculations.earlyAdopterDiscount)}</span>
                  </div>
                  {hasNIADiscount && (
                    <div className="flex justify-between text-yellow-700">
                      <span>NIA/Regional (10%)</span>
                      <span className="font-semibold">-{formatCurrency(calculations.niaDiscount)}</span>
                    </div>
                  )}
                  <div className="pt-1.5 border-t border-yellow-200 flex justify-between font-bold text-yellow-800">
                    <span>Total Savings</span>
                    <span>-{formatCurrency(calculations.earlyAdopterDiscount + calculations.niaDiscount)}/mo</span>
                  </div>
                </div>
              </div>
            )}
