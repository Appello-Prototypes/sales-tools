 ...
 * to provide comprehensive contact intelligence.
 * 
...
Use this tool to find similar contacts, understand customer history, research industry context, or gather any relevant background information. You can call this multiple times with different queries to gather comprehensive intelligence.`,
    input_schema: {
      type: 'object' as const,
      properties: {
        query: {
          type: 'string',
          description: 'The search query to send to ATLAS. Be specific and descriptive. Examples: "contacts in construction industry", "customer history for contacts in Ontario", "decision makers in enterprise sales"'
        }
      },
      required: ['query']
    }
  },
  {
    ...
    description: `Call this when you have gathered enough information and are ready to provide your final contact intelligence analysis. This signals that you are done investigating and ready to present your findings.`,
    input_schema: {
      type: 'object' as const,
      properties: {
        summary: {
  ...
  try {
    ...
    if (hubspotStatus.connected && hubspotStatus.tools) {
      ...
      for (const mcpTool of mcpTools) {
        // Skip tools that might conflict or aren't useful for contact analysis
...
function getFallbackHubspotTools(): Anthropic.Tool[] {
  return [
    ...
    },
    {
      ...
      description: 'Get associations between HubSpot objects (e.g., deals for a contact)',
      input_schema: {
        type: 'object' as const,
        properties: {
          objectType: {
...
const DEFAULT_AGENT_SYSTEM_PROMPT = `You are an expert sales intelligence analyst. Your job is to analyze contacts and provide actionable insights to help sales teams engage more effectively.

## Your Goal
Thoroughly analyze the given contact and provide comprehensive intelligence including:
- Contact engagement assessment (likelihood to engage)
- Key insights and observations about the contact
- Recommended next actions
- Risk factors to watch
- Opportunity signals
- Comparison to similar contacts
...
- Similar contacts and their engagement patterns
- Customer/company history  
- Industry insights
- Best practices for this type of contact
...
- Look up the company behind this contact
- Find deals associated with the contact
- Search for similar contacts (by role, industry, etc.)
- Understand contact history and associations

## Your Approach
1. **Investigate thoroughly** - Use the available tools to gather information. Don't just make one query - dig deeper. Ask follow-up questions. Look at the contact from multiple angles.

2. **Be curious** - What would a senior sales strategist want to know? What patterns exist? What worked for similar contacts? What risks exist?
...
4. **Cross-reference** - Compare what you find in ATLAS with the current contact data. Look for patterns, similarities, and differences.
...
async function getAgentConfig(): Promise<{
  ...
  if (cachedConfig && (now - configLastFetched) < CONFIG_CACHE_TTL) {
    return {
      systemPrompt: cachedConfig.agents.contact.systemPrompt,
      maxIterations: cachedConfig.agents.contact.maxIterations,
...
/**
 * Run the contact intelligence agent
 */
export async function runContactIntelligenceAgent(
  contactContext: ContactContext,
  hubspotApiKey: string,
  sendProgress: (progress: AgentProgress) => void,
  ...
  // Initial user message with contact context
  const initialMessage = `Analyze this contact and provide comprehensive intelligence:

## Contact Information
- **Contact ID**: ${contactContext.contactId}
- **Name**: ${contactContext.fullName}
- **Email**: ${contactContext.email || 'Not provided'}
- **Job Title**: ${contactContext.jobTitle || 'Not provided'}
- **Phone**: ${contactContext.phone || 'Not provided'}
${contactContext.companyId ? `- **Company ID**: ${contactContext.companyId}` : ''}
${contactContext.companyName ? `- **Company**: ${contactContext.companyName}` : ''}

## Your Task
1. Use the available tools to gather relevant information about this contact
2. Search ATLAS for similar contacts, company history, and relevant insights
3. Investigate the contact from multiple angles (company, deals, engagement history)
...
Be thorough - make multiple tool calls if needed. Explain your reasoning for each investigation.`;
  ...
  sendProgress({
    ...
    timestamp: new Date(),
    data: {
      message: 'Starting contact intelligence analysis...',
      contactName: contactContext.fullName,
          ...
          if (response.stop_reason === 'end_turn' && !response.content.some(b => b.type === 'tool_use')) {
            ...
            try {
              ...
              if (jsonMatch) {
                ...
                const rawJson = block.text.match(/\{[\s\S]*"engagementScore"[\s\S]*\}/);
                if (rawJson) {
    ...
    try {
      const finalResponse = await client.messages.create({
        ...
        messages: [
          ...
          {
            role: 'user',
            content: `Now provide your final contact intelligence analysis in JSON format:

{
  ...
  "similarContactsAnalysis": "<analysis text>",
      ...
      if (textBlock && textBlock.type === 'text') {
        try {
          const jsonMatch = textBlock.text.match(/```json\s*([\s\S]*?)```/) || 
                           textBlock.text.match(/\{[\s\S]*"engagementScore"[\s\S]*\}/);
          if (jsonMatch) {
